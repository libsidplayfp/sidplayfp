...
Set bit precision for file saving. The default is 32 (32 bit
float), but can be set to 16 to create 16 bit signed
samples. Other values will cause invalid output.
...
file will be <name> with no tune number added and <name> IS NOT
checked for or appended with a legal wav file extension.
...
=item Copyright (C) 2010-2019 Leandro Nini
